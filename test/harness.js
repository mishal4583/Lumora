...
(function(){
  ...
  var ok = tryUpgradeJarCap('simple'); // costs 100 at tier 0 (FINAL ECONOMY LOCK)
...
(function(){
  ...
  // E26 Round 2 (Jar Progression Value Rebalance): Aurora's Light Value
  // ceiling is now 2.30x (up from 2.00x) and Simple's own base dropped to
  // 0.55x (from 0.65x) -- both deliberate, part of the same "cheap jars
  // weaker, endgame jar stronger" redesign. Coin Value's own ceiling
  // (1.75x) is untouched by this phase. The ENTIRE multiplier stack must
  // still stay BOUNDED (not open-ended), just at the new, wider range.
  var auroraJar = JARS.find(function(j){ return j.key === 'aurora'; });
  __check('Scenario E: Aurora\\'s own maxed Light Value is exactly 2.30x (E26 Round 2\\'s raised ceiling), not open-ended', Math.abs(auroraJar.lightValueMax - 2.30) < 1e-9);
  var eStack = auroraJar.lightValueMax * 1.75; // Aurora Light Value maxed x Coin Value maxed (E2's 1.75x ceiling, untouched)
  __check('Scenario E: the full stacked multiplier (maxed Aurora Light Value x maxed Coin Value) stays bounded at 4.025x, not an unbounded snowball', Math.abs(eStack - 4.025) < 1e-9, 'stack=' + eStack);
  __check('Scenario E: total power range from weakest (Simple base 0.55x) to strongest possible (4.025x) is a bounded ~7.32x, not explosive', Math.abs(eStack / 0.55 - 7.318) < 0.01, 'ratio=' + (eStack / 0.55));
})();
...
coins = 100;
__check('a tier purchase succeeds once affordable, deducting exactly tier 0\\'s price (90, FINAL ECONOMY LOCK)', tryPurchaseTier('light') === true && coins === 10 && upgrades.lightTier === 1);
...
// FINAL ECONOMY LOCK (direct instruction): Simple's 8-tier Capacity price
// ladder is restored to its previously-approved, locked flat/escalating
// values (EXPLICIT_TIER_PRICES.simple.capacity =
// [100,100,125,125,125,150,150,150]) -- the E26 round 2 breakthrough/
// filler pricing is superseded. Capacity's own gameplay VALUE table
// (CAPACITY_LEVELS.simple = [4,4,5,5,6,6,6,7,7], base 4 -> max 7) is
// unaffected by this lock -- only prices reverted, stats stay put.
...
coins = 100;
__check('a jar capacity upgrade succeeds once affordable, deducting exactly the first upgrade\\'s price (100, locked)', tryUpgradeJarCap('simple') === true && coins === 0 && upgrades.jarCapTiers.simple === 1);
...
__check('Simple Glass Jar (base 4, max 7) is NOT maxed after only 2 of its 8 possible upgrades', jarCapMaxed(__simpleJar) === false);
// drive the remaining 6 upgrades (tiers 2-7: prices 125/125/125/150/150/150) to reach max
...
__check('the remaining 6 upgrades all succeed, spending exactly the sum of their own locked prices (125+125+125+150+150+150=825), and reach Simple Glass Jar\\'s own max (7)', __simpleCapOk === true && __simpleCapSpent === 825 && jarCurrentCapacity(__simpleJar) === 7 && jarCapMaxed(__simpleJar) === true, 'spent=' + __simpleCapSpent + ' coins=' + coins);
...
// jar carries its own real base capacity, not a small bonus.
// E26 Round 2: base capacities redesigned (4/8/12/16/21/28, up from
// 5/6/7/8/9/12) so each new jar's BASE already beats the previous jar's
// MAX (see this phase's own Transition Proof table).
JARS.forEach(function(j){ upgrades.ownedJars[j.key] = true; });
[['simple', 4], ['lantern', 8], ['moon', 12], ['crystal', 16], ['elder', 21], ['aurora', 28]].forEach(function(pair){
...
if (!YT) {
  ...
  upgrades.jarCapTiers.aurora = CAP_TIER_COUNT[__auroraJarForPersist.key]; // E26 Round 2: the real tier count, not maxCapacity-capacity -- Aurora's own capacity span (28->36) is now narrower than its real tier count (26), see CAPACITY_LEVELS' own comment
...
(function(){
  ...
  __check('jarCompareValues(capacity, Simple) with nothing upgraded reads base=current=4, max=7 (E26 Round 2 target) -- straight off JARS[], not a hardcoded UI value', capVals.base === 4 && capVals.current === 4 && capVals.max === 7, 'got=' + JSON.stringify(capVals));
  ...
  __check('jarCompareValues(capacity, Simple) reflects Simple\\'s own live upgrade progress (current=5) once tiers are actually owned', capValsAfter.current === 5, 'got=' + JSON.stringify(capValsAfter));
  __check('jarCompareValues(capacity, Aurora) is completely untouched by Simple\\'s own progress above -- per-jar independence, same proof shape as every other per-jar stat in this file', auroraCapVals.base === 28 && auroraCapVals.current === 28 && auroraCapVals.max === 36, 'got=' + JSON.stringify(auroraCapVals));
  var reachVals = jarCompareValues('reach', auroraJar);
  __check('jarCompareValues(reach, Aurora) reads Aurora\\'s own locked base/max (145/175) straight off JARS[]', reachVals.base === 145 && reachVals.max === 175, 'got=' + JSON.stringify(reachVals));
...
// E26 Round 2 (Jar Progression Value Rebalance): Simple's own base Reach/
// Magnet Reach/Duration dropped to 45/110/3.5s (from 55/130/4.0) -- the
// starter jar's baseline is now deliberately weaker so upgrading it can
// never approach the next jar's own base (see this phase's own Transition
// Proof table).
__check('jarReachForRun/magnetReachForRun return exactly Simple\\'s own bases (45/110) when Jar Reach/Magnet Reach are not owned', jarReachForRun() === 45 && magnetReachForRun() === 110, 'reach=' + jarReachForRun() + ' magnetReach=' + magnetReachForRun());
__check('magnetDurationForRun returns exactly Simple\\'s own base (3.5s) when Magnet Duration is not owned', Math.abs(magnetDurationForRun(5) - 3.5) < 1e-9, 'got=' + magnetDurationForRun(5));
...
// E26 Round 2: Simple Glass Jar's ceiling is now 7 (down from 13, target
// 4->7), still 8 possible upgrades (level count preserved) -- capacity no
// longer climbs +1 per tier, it reads CAPACITY_LEVELS.simple directly
// (several tiers repeat the same integer by design, see that constant's
// own comment).
upgrades.equippedJar = 'simple'; upgrades.jarCapTiers.simple = 0;
[1, 2, 8].forEach(function(n, idx){
  upgrades.jarCapTiers.simple = n;
  var expected = CAPACITY_LEVELS.simple[n];
  __check('Simple Glass Jar upgrade ' + n + ': cumulative capacity ' + expected, jarCapacityForRun() === expected, 'got=' + jarCapacityForRun());
});
__check('Simple Glass Jar is maxed at 7 after exactly 8 upgrades, and a 9th grants nothing further (capped, not overshooting)', jarCapMaxed(JARS.find(function(j){ return j.key === 'simple'; })) === true && (function(){ upgrades.jarCapTiers.simple = 99; var v = jarCapacityForRun(); upgrades.jarCapTiers.simple = 8; return v === 7; })());
upgrades.jarCapTiers.simple = 0;
// E26 Round 2: Aurora Jar's ceiling is now 36 (target 28->36), still 26
// possible upgrades (level count preserved) -- capacity no longer climbs
// flat +1/tier, it reads CAPACITY_LEVELS.aurora directly.
...
[1, 5, 26].forEach(function(n){
  ...
  __check('Aurora Jar upgrade ' + n + ': cumulative capacity ' + CAPACITY_LEVELS.aurora[n], jarCapacityForRun() === CAPACITY_LEVELS.aurora[n], 'got=' + jarCapacityForRun());
});
__check('Aurora Jar is maxed at exactly 36 (its own ceiling, not Simple\\'s or any shared one)', jarCapMaxed(JARS.find(function(j){ return j.key === 'aurora'; })) === true);
__check('switching the equipped jar back to Simple reads Simple\\'s OWN progress (still 0), not Aurora\\'s -- proving progress is per-jar, not shared', (function(){ upgrades.equippedJar = 'simple'; var v = jarCapacityForRun(); upgrades.equippedJar = 'aurora'; return v === 4; })());
...
// Value are still all per-jar. E26 Round 2: reach/magnetReach/lightValue
// no longer use a flat per-tier step at all -- they read STAT_LEVELS' own
// explicit (mildly accelerating) curve. Duration still uses the flat-step
// formula, now against its own new base/max (3.5->4.25s). Reach/Magnet
// Reach/Light Value ceilings all moved this phase (55->45/64->55,
// 130->110/154->130, 0.65->0.55/0.95->0.70) -- see JARS' own comment.
var __simpleJarStat = JARS.find(function(j){ return j.key === 'simple'; });
upgrades.reachTiers.simple = 1;
__check('Jar Reach tier 1: STAT_LEVELS curve value (45.186), Magnet Reach untouched (110)', Math.abs(jarReachForRun() - 45.186) < 1e-9 && magnetReachForRun() === 110, 'jarReach=' + jarReachForRun());
upgrades.reachTiers.simple = 32;
__check('Jar Reach tier 32 (maxed): reaches Simple\\'s own new ceiling (55)', jarReachForRun() === 55 && jarStatMaxed('reach', __simpleJarStat));
upgrades.reachTiers.simple = 0;

upgrades.magnetReachTiers.simple = 1;
__check('Magnet Reach tier 1: STAT_LEVELS curve value (110.517), Jar Reach untouched (45)', Math.abs(magnetReachForRun() - 110.517) < 1e-9 && jarReachForRun() === 45, 'magnetReach=' + magnetReachForRun());
upgrades.magnetReachTiers.simple = 24;
__check('Magnet Reach tier 24 (maxed): reaches Simple\\'s own new ceiling (130)', magnetReachForRun() === 130 && jarStatMaxed('magnetReach', __simpleJarStat), 'magnetReach=' + magnetReachForRun());
upgrades.magnetReachTiers.simple = 0;

upgrades.durationTiers.simple = 1;
__check('Magnet Duration tier 1: base+0.02778s (3.52778s)', Math.abs(magnetDurationForRun(5) - 3.52778) < 1e-9, 'got=' + magnetDurationForRun(5));
upgrades.durationTiers.simple = 27;
__check('Magnet Duration tier 27 (maxed): reaches Simple\\'s own new ceiling (4.25s)', Math.abs(magnetDurationForRun(5) - 4.25) < 1e-9 && jarStatMaxed('duration', __simpleJarStat));
upgrades.durationTiers.simple = 0;

upgrades.lightValueTiers.simple = 1;
__check('Light Value (per-jar) tier 1: STAT_LEVELS curve value (0.5548x)', Math.abs(jarCurrentStat('lightValue', __simpleJarStat) - 0.5548) < 1e-9, 'got=' + jarCurrentStat('lightValue', __simpleJarStat));
upgrades.lightValueTiers.simple = 20;
__check('Light Value (per-jar) tier 20 (maxed): reaches Simple\\'s own new ceiling (0.70x)', Math.abs(jarCurrentStat('lightValue', __simpleJarStat) - 0.70) < 1e-9 && jarStatMaxed('lightValue', __simpleJarStat), 'got=' + jarCurrentStat('lightValue', __simpleJarStat));
...
// (Aurora's own untouched base is 145, not Simple's 45).
upgrades.ownedJars.aurora = true;
upgrades.reachTiers.simple = 2;
var __auroraJarStat = JARS.find(function(j){ return j.key === 'aurora'; });
__check('upgrading Simple\\'s Jar Reach leaves Aurora\\'s Jar Reach at its own untouched (and higher) base', Math.abs(jarCurrentStat('reach', __simpleJarStat) - 45.412) < 1e-9 && jarCurrentStat('reach', __auroraJarStat) === 145, 'simple=' + jarCurrentStat('reach', __simpleJarStat) + ' aurora=' + jarCurrentStat('reach', __auroraJarStat));
upgrades.reachTiers.simple = 0;
...
// FINAL ECONOMY LOCK (direct instruction): Coin Value's price ladder is
// now the explicit COIN_VALUE_PRICES table (replacing bandedTierCost()
// entirely) -- tier0 is 90 (was 60), and levels 58-60 (index 57-59) are
// deliberately flat at 2590 (locked, per explicit confirmation, since the
// spec text only defined levels 1-58) -- so "strictly increasing" no
// longer holds at the very end; "never decreases" (monotonic
// non-decreasing) is the real invariant now.
__check('Coin Value\\'s price curve is the FINAL ECONOMY LOCK shape -- 60 levels, tier0 = 90, monotonically non-decreasing throughout (flat only at the locked 58-60 tail, never decreasing)', (function(){
  var p = TIER_LINES.light.prices;
  if (p.length !== 60 || p[0] !== 90) return false;
  for (var i = 1; i < p.length; i++) { if (p[i] < p[i - 1]) return false; }
  return true;
})(), 'prices=' + JSON.stringify(TIER_LINES.light.prices));
__check('Coin Value\\'s full 60-tier price table matches the FINAL ECONOMY LOCK table exactly (spot-checked at tier 0/1/2/59) and totals 48,325', TIER_LINES.light.prices[0] === 90 && TIER_LINES.light.prices[1] === 95 && TIER_LINES.light.prices[2] === 100 && TIER_LINES.light.prices[59] === 2590 && TIER_LINES.light.prices.reduce(function(a, b){ return a + b; }, 0) === 48325, 'got=' + JSON.stringify([TIER_LINES.light.prices[0], TIER_LINES.light.prices[1], TIER_LINES.light.prices[2], TIER_LINES.light.prices[59]]));
...
(function(){
  ...
  upgrades[field] = 0;
  [90, 95, 100].forEach(function(price, i){
    coins = price - 1;
  ...
  upgrades[field] = 59; coins = 2589;
  __check('light tier 59 (the final level) is rejected one coin short (price 2590)', tryPurchaseTier('light') === false && upgrades[field] === 59);
  coins = 2590;
  __check('light tier 59 succeeds at exactly its price (2590), reaching the 60-level max', tryPurchaseTier('light') === true && coins === 0 && upgrades[field] === 60);
...
// FINAL ECONOMY LOCK (direct instruction): reach/magnetReach are restored
// to their previously-approved (E26 round 1) locked prices; lightValue
// uses the FINAL APPROVED replacement table (brand new numbers). Duration
// has no hand-authored table and stays on the E25-rebalanced
// bandedTierCost() formula, unaffected by this lock. Tier COUNTS are
...
[
  ...
  { statKey: 'lightValue', early: [10, 10, 10], lastIdx: 19, lastPrice: 250 }
...
// FINAL ECONOMY LOCK (direct instruction): Simple's 8-tier Capacity line
// is locked at [100,100,125,125,125,150,150,150]
// (EXPLICIT_TIER_PRICES.simple.capacity).
upgrades.jarCapTiers.simple = 0;
...
// FINAL ECONOMY LOCK (direct instruction): tier-0 capacity prices below
// are EXPLICIT_TIER_PRICES[jar].capacity[0] for each jar, locked.
...
// FINAL ECONOMY LOCK: tier-1 capacity prices below are EXPLICIT_TIER_PRICES[jar].capacity[1], locked.
...
// stat key.
// FINAL ECONOMY LOCK: tier-0 reach prices below are EXPLICIT_TIER_PRICES[jar].reach[0], locked.
...
__check('jarStatUpgradeCost(lightValue) tier 0 for Aurora applies the FINAL APPROVED price (230)', jarStatUpgradeCost('lightValue', __auroraJarCost) === 230, 'got=' + jarStatUpgradeCost('lightValue', __auroraJarCost));
...
// FINAL ECONOMY LOCK (direct instruction): expected array is
// EXPLICIT_TIER_PRICES.aurora.capacity verbatim, the locked E26 round 1
// values, not the round 2 breakthrough/filler prices.
(function(){
  var auroraJar = JARS.find(function(j){ return j.key === 'aurora'; });
  ...
  __check('the total cost to fully max Aurora\\'s Capacity line (199,450 coins, FINAL ECONOMY LOCK)', got.reduce(function(a, b){ return a + b; }, 0) === 199450, 'total=' + got.reduce(function(a, b){ return a + b; }, 0));
  upgrades.jarCapTiers.aurora = 0;
})();
...
reset();
__check('a jar capacity upgrade sets the round-start capacity from CAPACITY_LEVELS (still 4 at tier 1 -- a filler level)', S.cap === 4, 'cap=' + S.cap);
...
// E26 Round 2: Simple Glass Jar's own ceiling dropped this phase (45
// base -> 55 max, down from 55->64) -- a firefly at 52px sits outside the
// new base radius but inside the new maxed one.
upgrades.reachTiers.simple = 0;
var farFly = placeFlyAtDistance('y', 52);
__stepFrame(16);
__check('a firefly at 52px stays out of range at Simple\\'s new base 45 radius (Jar Reach not owned)', farFly.state === 'drift', 'state=' + farFly.state);
upgrades.reachTiers.simple = 32;
var reachedFly = placeFlyAtDistance('y', 52);
__stepFrame(16);
__check('the same 52px firefly locks on once Jar Reach is maxed (55 radius, E26 Round 2)', reachedFly.state === 'locked' || reachedFly.state === 'caught', 'state=' + reachedFly.state);
upgrades.reachTiers.simple = 0;
...
__stepFrame(16);
__check('magnet buff duration is exactly Simple\\'s own new base (3.5s) when Magnet Duration is not owned', Math.abs(S.magnetT - 3.5) < 0.05, 'magnetT=' + S.magnetT);
upgrades.durationTiers.simple = 1;
reset(); screen = 'play'; paused = false;
spawnMagnet();
S.magnet.x = S.jar.x; S.magnet.y = S.jar.y - 14;
__stepFrame(16);
__check('magnet buff duration is exactly Simple\\'s new base+0.02778s (3.52778s) once Magnet Duration tier 1 is owned', Math.abs(S.magnetT - 3.52778) < 0.01, 'magnetT=' + S.magnetT);
upgrades.durationTiers.simple = 0;
...
__check('Curious delivers exactly its base coin value x Simple\\'s Light Value (0.65*0.55=0.3575) with Coin Value not owned', Math.abs((coins + coinFraction) - (totalBeforeLight + 0.3575)) < 1e-9, 'total=' + (coins + coinFraction));
reset(); screen = 'play'; paused = false;
...
__check('Curious delivers a Coin-Value-boosted value (0.65*0.55*1.75=0.625625) at Coin Value\\'s new max (level 60)', Math.abs((coins + coinFraction) - (totalBeforeLight2 + 0.625625)) < 1e-9, 'total=' + (coins + coinFraction));
upgrades.lightTier = 0;
...
(function(){
  ...
  __check('switching the equipped jar after migration does not transfer the migrated tier to the new jar', Math.abs(simpleReach - 45.412) < 1e-9 && lanternReach === 60 && Math.abs(simpleReachAfterSwitch - 45.412) < 1e-9, 'simple=' + simpleReach + ' lantern=' + lanternReach + ' simpleAfter=' + simpleReachAfterSwitch);
  upgrades.reachTiers.simple = 0;
})();
...
(function(){
  ...
  upgrades.jarCapTiers.aurora = CAP_TIER_COUNT.aurora; // E26 Round 2: the real tier count, not maxCapacity-capacity
...
// E23: S.landmarkAnim now also requires S.nightDelivered (temporary,
// THIS-session progress) to independently reach the same threshold -- see
// the tick's own comment in update() -- so this helper also sets
// S.nightDelivered=bestVal, simulating "this session's own play has also
// (re-)reached this point," which is what TEST A-E here have always
// actually been probing (does the rendering pipeline correctly follow
// restoration state end to end). The NEW, genuinely distinct case this
// phase adds -- permanently restored but S.nightDelivered=0 at a fresh
// night's start -- gets its own dedicated test block right after these,
// using this same helper's pieces directly rather than through it.
...
(function(){
  ...
  // Now restore that SAME light's landmark group (index 2, threshold 150)
  // AND reach that same threshold with tonight's own delivered count (E23:
  // permanent restoration alone is no longer enough to light this pixel --
  // see the landmarkAnim tick's own comment) -- confirm its dot appears at full brightness
  villageProgression.villages[0].bestNightDelivered = 150;
  S.nightDelivered = 150;
  rebuildVillageMilestones(villageProgression.villages[0]);
...
// E23 (direct instruction, correcting the E21-era assumption right below):
// reset() immediately zeroes the ANIMATION (S.landmarkAnim), not the
// PERMANENT restoration state -- the milestones stay restored throughout,
// exactly as before. But unlike the OLD "awaken fade" behavior this
// replaces, these landmarks must NOT auto-ease back to full brightness on
// their own just because they were restored on a past night -- "restored"
// (permanent) and "illuminated tonight" (temporary, S.nightDelivered-
// gated) are now two separate conditions that must BOTH hold. A fresh
// night starts visually dark regardless of permanent history, and must
// stay dark until THIS session's own deliveries re-cross each threshold.
__check('TEST F/E23: immediately after reset(), the landmark ANIMATION dips to 0 (no instant full-brightness pop)...', LUMORA_LANDMARK_GROUPS.every(function(_, gi){ return !S.landmarkAnim[gi]; }));
__check('TEST F/E23: ...while the PERMANENT milestone state is untouched -- Buildings 1-6 are still restored, not temporarily unrestored', getRestoredMilestones('lumora').length === 6);
...
__check('TEST F/E23: buildings 1-6 do NOT automatically ease back in on their own -- permanent restoration no longer auto-illuminates a fresh night with zero deliveries', LUMORA_LANDMARK_GROUPS.every(function(_, gi){ return !S.landmarkAnim[gi]; }));
...
for (var fnv2 = 0; fnv2 < 90; fnv2++) __stepFrame(16); // let the tick settle
...
__check('TEST F: Buildings 1-6 stay PERMANENTLY restored, 7-8 stay locked -- no relocking from the worse night', getRestoredMilestones('lumora').length === 6 && villageProgression.villages[0].milestones[6].state === 'locked' && villageProgression.villages[0].milestones[7].state === 'locked');
__check('TEST F/E23: but tonight\\'s own illumination shows NOTHING lit -- 40 delivered this session never even reaches Building 1\\'s own 50 threshold, even though Building 1 is permanently restored', LUMORA_LANDMARK_GROUPS.every(function(_, gi){ return !S.landmarkAnim[gi]; }));
// Now re-earn PAST Building 1's threshold (50) THIS session, but not
// Building 2's (100) -- proves tonight's illumination tracks tonight's OWN
// progress landmark-by-landmark, independent of the permanent record.
for (var fN2 = 40; fN2 < 60; fN2++) { grantVillageProgress(); }
for (var fnv3 = 0; fnv3 < 90; fnv3++) __stepFrame(16);
__check('TEST F/E23: at 60 delivered this session, ONLY Building 1 (threshold 50) is illuminated tonight -- Buildings 2-6, though permanently restored, stay dark until tonight\\'s own count reaches their own thresholds too', S.landmarkAnim[0] > 0.9 && [1, 2, 3, 4, 5, 6, 7].every(function(gi){ return !S.landmarkAnim[gi]; }));
__check('TEST F/E23: the permanent record is completely unaffected by any of this -- still 300, still 6/8', villageProgression.villages[0].bestNightDelivered === 300 && getRestoredMilestones('lumora').length === 6);

// ===== E23's own exact test matrix (report section 13), under this
// report's own best=240 numbers -- distinct scenarios from TEST F above,
// restated for direct traceability against the spec. =====
(function(){
  var v = villageProgression.villages[0];

  // TEST A -- fresh Lumora: 0/8, no illumination, all locked.
  v.bestNightDelivered = 0; S.nightDelivered = 0;
  rebuildVillageMilestones(v);
  screen = 'village'; paused = false;
  for (var a = 0; a < 90; a++) __stepFrame(16);
  __check('E23 TEST A: fresh Lumora (best=0, current=0) -- 0/8 restored, zero landmark illumination', getRestoredMilestones('lumora').length === 0 && LUMORA_LANDMARK_GROUPS.every(function(_, gi){ return !S.landmarkAnim[gi]; }));

  // TEST B -- restored but new night: best=240, current=0.
  v.bestNightDelivered = 240; S.nightDelivered = 0;
  rebuildVillageMilestones(v);
  for (var b = 0; b < 90; b++) __stepFrame(16);
  __check('E23 TEST B: best=240, current=0 -- 4/8 PERMANENTLY restored (Buildings 1-4 still exist/unlocked)', getRestoredMilestones('lumora').length === 4);
  __check('E23 TEST B: Buildings 5-8 remain locked', v.milestones.slice(4).every(function(m){ return m.state === 'locked'; }));
  __check('E23 TEST B CRITICAL: current-night illumination starts at the dormant state -- NOT 4/8 worth of buildings lit up just because they are permanently restored', LUMORA_LANDMARK_GROUPS.every(function(_, gi){ return !S.landmarkAnim[gi]; }));

  // TEST C -- current night progress: best=240, current=50 (crosses only Building 1's own 50 threshold).
  S.nightDelivered = 50;
  for (var c = 0; c < 90; c++) __stepFrame(16);
  __check('E23 TEST C: permanent state remains 4/8 (unaffected by tonight\\'s own count)', getRestoredMilestones('lumora').length === 4);
  __check('E23 TEST C: current-night illumination reflects ONLY tonight\\'s own 50 -- Building 1 lit, Buildings 2-8 (including the still-locked ones) dark', S.landmarkAnim[0] > 0.9 && [1, 2, 3, 4, 5, 6, 7].every(function(gi){ return !S.landmarkAnim[gi]; }));

  // TEST D -- current night HIGHER than old best: best=240, current=250 (crosses Building 5, previously un-reached).
  // grantVillageProgress() itself is what performs finalization-on-delivery
  // in this codebase (there is no separate deferred "end of night" step --
  // the live high-water-mark ratchet fires the instant a new record is hit).
  S.nightDelivered = 249;
  grantVillageProgress(); // 249 -> 250, a genuinely NEW all-time record -- ratchets bestNightDelivered for real
  __check('E23 TEST D: crossing 250 for the first time (via the real delivery flow) updates the permanent record to 250', v.bestNightDelivered === 250);
  __check('E23 TEST D: Building 5 becomes permanently restored -- 5/8 total', getRestoredMilestones('lumora').length === 5);
  for (var d = 0; d < 90; d++) __stepFrame(16);
  __check('E23 TEST D: current-night illumination reflects the same 250 -- Buildings 1-5 lit tonight, 6-8 dark', [0, 1, 2, 3, 4].every(function(gi){ return S.landmarkAnim[gi] > 0.9; }) && [5, 6, 7].every(function(gi){ return !S.landmarkAnim[gi]; }));

  // TEST E -- worse night: best=250 (from TEST D above), a new night with only 80 delivered.
  reset();
  S.isNewNight = false; S.newNightT = 999; screen = 'village'; paused = false;
  for (var e = 0; e < 80; e++) grantVillageProgress();
  for (var e2 = 0; e2 < 90; e2++) __stepFrame(16);
  __check('E23 TEST E: best remains 250 after a worse (80-delivery) night -- no relocking', v.bestNightDelivered === 250 && getRestoredMilestones('lumora').length === 5);
  __check('E23 TEST E: tonight\\'s own 80 deliveries clear Building 1\\'s 50 threshold but not Building 2\\'s 100 -- only Building 1 lit tonight, the permanent record stays 5/8 throughout', S.landmarkAnim[0] > 0.9 && [1, 2, 3, 4, 5, 6, 7].every(function(gi){ return !S.landmarkAnim[gi]; }));

  // E23 defensive/robustness check: the tick genuinely requires BOTH
  // conditions (permanently restored AND tonight's count past the
  // threshold), not just whichever one happens to usually move together in
  // real play. Forces a milestone's state to 'locked' by hand while
  // S.nightDelivered is already comfortably past its threshold -- proves a
  // truly locked building could never light up even in a hypothetical
  // future bug that let S.nightDelivered outrun the permanent ratchet.
  (function(){
    S.nightDelivered = 999; // already past every threshold
    v.milestones[7].state = 'locked'; // force Building 8 (500) locked despite S.nightDelivered qualifying
    for (var g = 0; g < 90; g++) __stepFrame(16);
    __check('E23 (defense-in-depth): a milestone forced to LOCKED never illuminates even when S.nightDelivered alone would qualify -- both conditions are genuinely required, not just one', !S.landmarkAnim[7]);
    rebuildVillageMilestones(v); // restore the real, correctly-derived state for anything running after
  })();
})();
...
// E23 #12: Moonfall verification -- its own drawVillage() branch never had
// this bug in the first place (it already reveals structures via
// S.nightDelivered/pct, never the permanent bestNightDelivered -- see that
// branch's own comment), and this phase's fix only touches the
// currentVillage==='lumora' branch of the landmark tick, so Moonfall's
// rendering path is untouched by construction. This proves that directly:
// Moonfall's own pct is temporary (S.nightDelivered-driven, resets with
// reset()) regardless of how high its OWN permanent record is.
(function(){
  var mv = villageProgression.villages[1];
  mv.bestNightDelivered = 425; // a real, permanent Moonfall record
  rebuildVillageMilestones(mv);
  reset(); // a fresh night -- S.nightDelivered zeroes
  villageProgression.currentVillage = 'moonfall';
  var pctFresh = mv.completionThreshold != null ? Math.min(1, Math.max(0, S.nightDelivered / mv.completionThreshold)) : 1;
  __check('E23 #12: Moonfall\\'s permanent record (425, 4/8) is untouched by a fresh night starting', mv.bestNightDelivered === 425 && getRestoredMilestones('moonfall').length === 4);
  __check('E23 #12: Moonfall\\'s own temporary reveal pct is genuinely 0 on a fresh night, exactly like Lumora\\'s landmarks now are -- it was never driven by the permanent record to begin with', pctFresh === 0 && S.nightDelivered === 0);
  var moonfallDrawThrew = false;
  try { draw(); } catch (e) { moonfallDrawThrew = true; }
  __check('E23 #12: Moonfall\\'s own renderer draws this state without throwing -- Lumora\\'s landmark-tick fix does not touch Moonfall\\'s art at all', !moonfallDrawThrew);
  villageProgression.currentVillage = 'lumora';
...
return __tick(5).then(function(){
  ...
  // step, reconstructed against the OLD base/max frozen in
  // PRE_E26R2_STAT_RANGE) into their current equivalents.
  // E26 Round 2: Simple's reach/magnetReach/duration ceilings all
  // DROPPED this phase (e.g. reach max 64->55) -- old tier 1's
  // reconstructed value (reach 55+1*3=58, magnetReach 130+10=140,
  // duration 4.0+0.5=4.5) now EXCEEDS the new ceiling entirely (55/130/
  // 4.25), so "preserve or slightly increase power, never reduce it"
  // means fully maxing all three out (32/24/27 tiers) rather than landing
        ...
        return __tick(3).then(function(){
          ...
          return __tick(5).then(function(){
            ...
            return __tick(5).then(function(){
              ...
              __check('Coin Value tier-0 purchase still costs exactly 90 (FINAL ECONOMY LOCK price)', boughtOk === true && coins === 910 && upgrades.lightTier === 1, 'coins=' + coins);
      ...
      return __tick(5).then(function(){
        ...
        return __tick(5).then(function(){
          ...
          return __tick(5).then(function(){
            ...
            __check('normal Coin Value purchase still works exactly as before -- unaffected by the Workshop favor card existing', tryPurchaseTier('light') === true && coins === 910 && upgrades.lightTier === 1);
...
S.coinsEarnedThisNight = 140;
// E38: Collector's reward (the flat Workshop Token) was removed entirely --
// it now has no reward at all, so contractPayoutCoins() being 0 is simply
// "no coin reward", not "a non-coin reward shown elsewhere" any more.
__check('Collector has no coin multiplier and no reward at all -- contractPayoutCoins() is 0', contractPayoutCoins() === 0);
activeContract = -1;
__check('no contract: contractPayoutCoins() is 0', contractPayoutCoins() === 0);

// ---- E38: Workshop Tokens are gone -- the state variable itself no longer exists ----
__check('E38: workshopTokens is no longer a defined variable anywhere in scope -- not just zeroed, genuinely removed', typeof workshopTokens === 'undefined');

// ---- finalizeNight(): records which contract ran; Collector grants no reward, so no token/balance of any kind can be created ----
upgrades.tutorialDone = true;
reset(); screen = 'play'; paused = false;
activeContract = 1; // rush -- a coin-multiplier contract, unaffected by the removal
...
reset(); screen = 'play'; paused = false;
activeContract = 3; // collector -- E38: grants nothing now, no token, no replacement reward
var coinsBeforeCollectorNight = coins, contractsCompletedBefore2 = contractsCompleted.length;
var threwOnCollectorFinalize = false;
try { finalizeNight(); } catch (e) { threwOnCollectorFinalize = true; }
__check('completing a Collector night does not throw now that its token reward is gone', !threwOnCollectorFinalize);
__check('completing a Collector night still records it in contractsCompleted (that tracking is unrelated to the removed reward)', contractsCompleted.length === contractsCompletedBefore2 + 1 && contractsCompleted[contractsCompleted.length - 1] === 'collector');
__check('completing a Collector night grants no coins either -- no replacement reward was added', coins === coinsBeforeCollectorNight);
...
__check('Test 2/3: existing ownership and upgrade tiers resolve exactly as before -- Phase 10 reads currentJar()/jarCurrentCapacity() but never writes to them', upgrades.ownedJars.crystal === true && upgrades.jarCapTiers.crystal === 3 && capBefore === CAPACITY_LEVELS.crystal[3]);
...
// E26 Round 2 (Jar Progression Value Rebalance): Simple's own reach/
// magnetReach/duration ceilings all DROPPED this phase (reach max 64->55,
// magnetReach 154->130, duration 5.5->4.25) -- reconstructed against the
// frozen PRE_E26R2_STAT_RANGE (the OLD ceiling that was actually true for
// this save), old tier 1's value (reach 58, magnetReach 140, duration 4.5)
// now EXCEEDS the new ceiling entirely, so these three fully max out
// rather than landing on some smaller in-range tier -- "preserve or
// increase power" bottoms out at "give them everything the new, smaller
// jar can offer" once the new ceiling itself is below what they already
// had. This is an intentional, disclosed consequence of deliberately
// nerfing Simple's stats this phase, not a migration bug.
// ---- Reach migration ----
upgrades.reachTiers.simple = 1;
migrateEconomyV2();
__check('Reach migration: old tier 1 (value 58, now above Simple\\'s new 55 ceiling) migrates to the new max tier (32)', upgrades.reachTiers.simple === 32, 'got=' + upgrades.reachTiers.simple);
var __simpleJarE2 = JARS.find(function(j){ return j.key === 'simple'; });
__check('the migrated Reach tier reaches Simple\\'s new ceiling exactly (55) -- genuinely less than the old 58, because Simple\\'s own ceiling was deliberately lowered this phase', Math.abs(jarCurrentStat('reach', __simpleJarE2) - 55) < 1e-9, 'got=' + jarCurrentStat('reach', __simpleJarE2));
upgrades.reachTiers.simple = 0; upgrades.economyV2Migrated = false;

// ---- Magnet Reach migration ----
upgrades.magnetReachTiers.simple = 1;
migrateEconomyV2();
__check('Magnet Reach migration: old tier 1 (value 140, now above Simple\\'s new 130 ceiling) migrates to the new max tier (24)', upgrades.magnetReachTiers.simple === 24, 'got=' + upgrades.magnetReachTiers.simple);
upgrades.magnetReachTiers.simple = 0; upgrades.economyV2Migrated = false;

// ---- Duration migration ----
upgrades.durationTiers.simple = 1;
migrateEconomyV2();
__check('Duration migration: old tier 1 (value 4.5, now above Simple\\'s new 4.25 ceiling) migrates to the new max tier (27)', upgrades.durationTiers.simple === 27, 'got=' + upgrades.durationTiers.simple);
__check('the migrated Duration tier reaches Simple\\'s new ceiling exactly (4.25s) -- genuinely less than the old 4.5s, because Simple\\'s own ceiling was deliberately lowered this phase', Math.abs(jarCurrentStat('duration', __simpleJarE2) - 4.25) < 1e-9, 'got=' + jarCurrentStat('duration', __simpleJarE2));
upgrades.durationTiers.simple = 0; upgrades.economyV2Migrated = false;

// ---- Light Value migration: Simple's Light Value ceiling ROSE this phase (0.80->wait, base 0.65->0.55 dropped but max stayed reachable at 0.70) -- old tier 1's value (0.70) still fits within the new curve exactly at its own final level (20), so "never less" is fully honored here, unlike reach/magnetReach/duration above. ----
upgrades.lightValueTiers.simple = 1;
migrateEconomyV2();
__check('Light Value migration: old tier 1 (value 0.70) migrates to new tier 20 (the smallest new tier whose value is >= 0.70, never fewer)', upgrades.lightValueTiers.simple === 20, 'got=' + upgrades.lightValueTiers.simple);
...
JARS.forEach(function(jar){
  // capacity
  // E26 Round 2: capTiers is now CAP_TIER_COUNT, not maxCapacity-capacity
  // (capacity's own span can be narrower than its real tier count -- see
  // CAPACITY_LEVELS' own comment). Price is intentionally NOT monotonic
  // any more: a "breakthrough" level (capacity actually goes up) costs a
  // real progressive price, a "filler" level (capacity repeats) costs
  // substantially less -- so a price DROP right after a breakthrough is
  // expected and correct, not a bug. What must still hold: every price is
  // positive, and the capacity VALUE itself is monotonic non-decreasing
  // (checked separately below via CAPACITY_LEVELS).
  var capTiers = CAP_TIER_COUNT[jar.key];
  var prevCapValue = -1;
  for (var ct = 0; ct < capTiers; ct++) {
    upgrades.jarCapTiers[jar.key] = ct;
    var capPrice = jarCapUpgradeCost(jar);
    var capValue = jarCurrentCapacity(jar);
    ...
    __check(jar.key + ' capacity tier ' + ct + ' value never decreases from the previous tier', capValue >= prevCapValue, 'prev=' + prevCapValue + ' got=' + capValue);
    prevCapValue = capValue;
  }
...
function totalJarSink(jar){
  var total = 0;
  var capTiers = CAP_TIER_COUNT[jar.key]; // E26 Round 2: the real tier count, not maxCapacity-capacity
  upgrades.jarCapTiers[jar.key] = 0;
...
// E26 Round 2: Simple's own Light Value base dropped 0.65->0.55 (the
// starter jar is deliberately weaker this phase), so the beginner
// coins/catch formula and the resulting coins/night band both shift down
// with it -- an intended consequence of the redesign, not an accidental
// coin-income change (TYPES.y.coins itself, coinMultiplierForRun(), and
// every other income source are untouched).
__check('Beginner coins/catch (Curious, Simple jar, Coin Value untouched) matches the exact formula (0.65*0.55*1.0=0.3575)', Math.abs(beginnerPerCatch - 0.3575) < 1e-9, 'got=' + beginnerPerCatch);
var beginnerCoinsPerNight = beginnerPerCatch * 20; // ~20 catches/night, this phase's own established beginner estimate
__check('Beginner coins/night (~20 catches) lands in the new (E26 Round 2, Simple deliberately weaker) 6-20 coins/night band', beginnerCoinsPerNight >= 6 && beginnerCoinsPerNight <= 20, 'got=' + beginnerCoinsPerNight);
...
__check('the very first Capacity upgrade (Simple, tier 0) costs 100 (FINAL ECONOMY LOCK price)', firstCapCost === 100);
...
upgrades.tutorialDone = true;
var simpleJar = JARS.find(function(j){ return j.key === 'simple'; });
__check('jarCapUpgradeCost is still exactly the FINAL ECONOMY LOCK price (tier 0 = 100, unchanged by E3)', jarCapUpgradeCost(simpleJar) === 100);
__check('Coin Value tier-0 price is still exactly 90, unchanged by E3', TIER_LINES.light.prices[0] === 90);
...
(function(){
  ...
  try {
    JARS.forEach(function(jar){
      ...
      upgrades.jarCapTiers[jar.key] = CAP_TIER_COUNT[jar.key] - 1; // E26 Round 2: real tier count, not maxCapacity-capacity
      ...
      // fully maxed
      upgrades.jarCapTiers[jar.key] = CAP_TIER_COUNT[jar.key];
...
return __tick(5).then(function(){
  ...
  return __tick(5).then(function(){
    ...
    // E26 Round 2: an offset proportional to costMid, not a flat -15 --
    // capacity's own tier-5 price is much smaller now (25, a "filler"
    // level) than the old flat-formula price (150) this offset was
    // originally tuned against; a flat -15 would put the balance outside
    // the real 15% eligibility window entirely.
    var midShort = Math.max(1, Math.round(costMid * 0.08));
    coins = costMid - midShort;
    ...
    requestAlmostAffordable({ kind: 'jarCap', jarKey: 'simple' });
    coins += Math.ceil(midShort / 2); // the player earns coins from real gameplay WHILE the ad is playing, before the callback fires -- always less than midShort, so a real shortfall still remains at callback time
    return __tick(5).then(function(){
      __check('the actual reward reflects the balance AT CALLBACK TIME, never the stale amount calculated when the ad was requested', coins === costMid, 'coins=' + coins + ' cost=' + costMid);
      upgrades.jarCapTiers.simple = 0;
      ...
      return __tick(5).then(function(){
        ...
        return __tick(5).then(function(){
          ...
          return __tick(5).then(function(){
            ...
            upgrades.jarCapTiers.simple = CAP_TIER_COUNT.simple; // fully maxed (E26 Round 2: real tier count, not maxCapacity-capacity)
            coins = 0;
            ...
            // E26 Round 2: tier 27 (price 20), not tier 10 (price 5) -- reach's
            // own early tiers are now too cheap for a 10%-short offset to stay
            // under the real 15% eligibility threshold (1 coin short of 5 is
            // already 20%); a slightly later tier keeps this test meaningful.
            upgrades.reachTiers.simple = 27;
            var reachCost = jarStatUpgradeCost('reach', JARS.find(function(j){ return j.key === 'simple'; }));
            coins = reachCost - Math.round(reachCost * 0.1);
            __check('a per-jar stat line (reach) is Almost-Affordable-eligible using the exact same threshold function', almostAffordableEligible({ kind: 'jarStat', statKey: 'reach', jarKey: 'simple' }) === true, 'coins=' + coins + ' cost=' + reachCost);
            upgrades.reachTiers.simple = 0;
...
// contractsCompleted (originally also double-granting a Collector night's
// Workshop Token, before E38 removed that reward entirely). Fixed with a
// one-line idempotency guard (if(S.over)return;), mirroring
// jarCanAcceptCatch()'s own "check right before mutation" discipline. This
// test calls finalizeNight() directly, twice in a row -- the exact
// reachable double-fire shape -- rather than trying to choreograph two
// fireflies expiring in the same real frame, which is the same "drive the
// canonical function directly" approach objectiveProgress()/
// advanceChain()'s own existing tests already use.
scenario('lumora2-e15-finalizeNight-reentry', null, `
reset();
activeContract = 3; // Collector -- E38: no reward of any kind any more, but contractsCompleted tracking is still the thing under test here
var completedBefore = contractsCompleted.length, coinsBefore = coins;
finalizeNight();
var completedAfterFirst = contractsCompleted.length;
...
__check('repeated finalizeNight() calls remain fully idempotent', contractsCompleted.length === completedAfterFirst);
__check('E38: none of this ever grants coins either -- Collector has no reward, so there is nothing left to duplicate', coins === coinsBefore);
`);
...
__check('43: Firefly (lantern) capacity tier-1 upgrade cost is untouched by E16 (250, FINAL ECONOMY LOCK)', jarCapUpgradeCost(jarByKey('lantern')) === 250);
upgrades.jarCapTiers.moon = 0;
__check('44: Moon capacity tier-1 upgrade cost is untouched by E16 (500, FINAL ECONOMY LOCK)', jarCapUpgradeCost(jarByKey('moon')) === 500);
__check('45: Crystal capacity tier-1 upgrade cost is untouched by E16 (900, FINAL ECONOMY LOCK)', jarCapUpgradeCost(jarByKey('crystal')) === 900);
__check('46: Elder capacity tier-1 upgrade cost is untouched by E16 (1500, FINAL ECONOMY LOCK)', jarCapUpgradeCost(jarByKey('elder')) === 1500);
__check('47: Aurora capacity tier-1 upgrade cost is untouched by E16 (2500, FINAL ECONOMY LOCK)', jarCapUpgradeCost(jarByKey('aurora')) === 2500);
__check('Coin Value (the shared line) is completely untouched by E16', TIER_LINES.light.prices.length === 60 && TIER_LINES.light.prices[0] === 90);
...
// =====================================================================
// E25 Workshop Economy Rebalance V2: capacity base 25->55, per-jar-stat
// bases 16/18/12/20 -> 35/40/26/44, and JAR_ENDGAME_MULTIPLIER raised for
// the 4 cheaper jars only (simple/lantern/moon/crystal; elder/aurora
// unchanged). Audit finding: a single strong night (~687 coins observed
// live) could fund deep simultaneous progress on capacity AND every other
// per-jar line at once -- the fix raises the floor and, for cheap jars,
// the ceiling of the existing bandedTierCost() curve, without touching the
...
__check('1: capacity base price is 100 (Simple jar, FINAL ECONOMY LOCK)', jarCapUpgradeCost(JARS.find(function(j){ return j.key === 'simple'; })) === 100);
__check('2: reach/magnetReach tier-0 are 75/100 (Simple jar, locked); Light Value tier-0 is 10 (final approved); duration stays 26 (E25 formula, independent of base/max)', (function(){
  var j = JARS.find(function(jj){ return jj.key === 'simple'; });
  return jarStatUpgradeCost('reach', j) === 75 && jarStatUpgradeCost('magnetReach', j) === 100 && jarStatUpgradeCost('duration', j) === 26 && jarStatUpgradeCost('lightValue', j) === 10;
})());
...
// ---- 4/5/6: exact purchase price, exact deduction, insufficient-coins blocks purchase (Simple Capacity tier 0, the locked 100 price) ----
...
coins = 100;
__check('5: the purchase succeeds at exactly the locked price (100), deducting exactly that much', tryUpgradeJarCap('simple') === true && coins === 0 && upgrades.jarCapTiers.simple === 1);
...
// ---- 7: Almost Affordable uses the correct new shortfall against the locked price (Simple Capacity tier 0 = 100) ----
...
__check('7: Almost Affordable is eligible against the locked 100 price and computes the exact real shortfall (8)', almostAffordableEligible({ kind: 'jarCap', jarKey: 'simple' }) === true && almostAffordableShortfall({ kind: 'jarCap', jarKey: 'simple' }) === 8);
upgrades.jarCapTiers.simple = 0; coins = 0;
...
tryUpgradeJarCap('simple'); tryUpgradeJarCap('simple'); // 2 real tiers owned, spending real locked-price coins (100+100=200)
if (!YT) {
  ...
  __check('8b: coins spent on those real locked-price purchases (1000-100-100=800) are persisted to gk2_coins', __e25ReloadedCoins === 800, 'got=' + __e25ReloadedCoins);
}
upgrades.jarCapTiers.simple = 0;
...
// whole phase is about, so it's the cleanest possible proof here.
coins = 0; // the real (YT-mocked) load merge takes the MAX of in-memory vs. saved coins -- reset so the saved 200 is what actually lands, same discipline lumora2-e16-save-compat's fresh scenario gets for free
var preE25Save = {
  best: 25, coins: 200,
  upgrades: {
    tutorialDone: true,
    ownedJars: { simple: true, lantern: true }, equippedJar: 'simple',
    jarCapTiers: { simple: 3, lantern: 0, moon: 0, crystal: 0, elder: 0, aurora: 0 }
  }
};
...
// =====================================================================
// FINAL ECONOMY LOCK (direct instruction): Capacity/Range/Magnet Reach
// are restored to their previously-approved (E26 round 1) price tables
// and are now LOCKED -- the E26 round 2 breakthrough/filler Capacity
// pricing and the regenerated Range/Magnet/Light arrays from that same
// phase are both superseded. Light Value gets the FINAL APPROVED
// replacement table (brand new numbers). The Coin Value global line is
// now an explicit hand-approved array too, replacing its own
// bandedTierCost() formula. Duration is NOT covered -- it stays on the
// E25-rebalanced bandedTierCost() formula, fed the (unchanged-by-this-
// phase) duration base/max.
scenario('lumora2-e26-explicit-prices', { audioEnabled: true }, `
// ---- 1: every jar's Capacity tier-0/final-tier price matches EXPLICIT_TIER_PRICES exactly (FINAL ECONOMY LOCK values) ----
[
  ...
  // 1c/1d: "maxed" is defined by TIER COUNT (every level bought), not by
  // the capacity VALUE reaching its ceiling -- several jars' capacity
  // value actually reaches maxCapacity one tier before their real last
  // tier (a "filler" final level, see CAPACITY_LEVELS' own trailing
  // duplicate), and if "maxed" were value-based that real final tier
  // would be hidden behind "MAX LEVEL" in the shop UI, permanently
  // unpurchasable. Tier-count-based "maxed" keeps it buyable.
  ...
  __check('1d: buying that real final tier succeeds and reaches ' + cfg.key + '\\'s own Capacity max exactly', __finalBought === true && jarCapMaxed(jar) === true && jarCurrentCapacity(jar) === jar.maxCapacity);
  upgrades.jarCapTiers[cfg.key] = 0; coins = 0;
});

// ---- 2: every jar's Reach/Magnet Reach tier-0 price matches the LOCKED table exactly; Light Value uses the FINAL APPROVED table; Duration is untouched (still the E25 formula) ----
[
  { key: 'simple', reach: 75, magnetReach: 100, lightValue: 10 },
  { key: 'lantern', reach: 200, magnetReach: 275, lightValue: 10 },
  { key: 'moon', reach: 400, magnetReach: 550, lightValue: 10 },
  { key: 'crystal', reach: 700, magnetReach: 1000, lightValue: 30 },
  { key: 'elder', reach: 1200, magnetReach: 1650, lightValue: 100 },
  { key: 'aurora', reach: 2000, magnetReach: 2800, lightValue: 230 }
].forEach(function(cfg){
  var jar = JARS.find(function(j){ return j.key === cfg.key; });
...
(function(){
  ...
  __check('3c: Aurora Magnet Reach final (25th) tier costs 18000, its last locked price', __auroraMagnetFinalCost === 18000, 'got=' + __auroraMagnetFinalCost);
...
// ---- 4: Duration has no hand-authored price table -- its PRICE formula is untouched (independent of base/max), only its base/max stat endpoints moved (an earlier phase, unaffected by this lock) ----
__check('4: Simple Duration tier-0 price is still 26 (E25 formula is independent of durationBase/Max)', jarStatUpgradeCost('duration', JARS.find(function(j){ return j.key === 'simple'; })) === 26);
__check('4b: Aurora Duration tier-0 price is still 39 (E25 formula is independent of durationBase/Max)', jarStatUpgradeCost('duration', JARS.find(function(j){ return j.key === 'aurora'; })) === 39);
__check('4c: Simple Duration\\'s own base/max endpoints are the approved target (3.5/4.25)', Math.abs(jarStatBase('duration', JARS.find(function(j){ return j.key === 'simple'; })) - 3.5) < 1e-9 && Math.abs(jarStatMax('duration', JARS.find(function(j){ return j.key === 'simple'; })) - 4.25) < 1e-9);

// ---- 5: jar unlock prices and village thresholds are untouched by this lock; Coin Value now uses the FINAL APPROVED explicit table ----
...
__check('5b: Coin Value (TIER_LINES.light) has 60 levels, tier-0 = 90 (FINAL ECONOMY LOCK)', TIER_LINES.light.prices.length === 60 && TIER_LINES.light.prices[0] === 90);
__check('5c: Coin Value levels 59-60 are locked flat at level 58\\'s own price (2590), per explicit confirmation -- not the pre-lock formula\\'s 1650/1725 (which would have made the ladder non-monotonic)', TIER_LINES.light.prices[57] === 2590 && TIER_LINES.light.prices[58] === 2590 && TIER_LINES.light.prices[59] === 2590);
__check('5d: the full Coin Value ladder is monotonically non-decreasing end to end', TIER_LINES.light.prices.every(function(p, i){ return i === 0 || p >= TIER_LINES.light.prices[i - 1]; }));
`);
...
// =====================================================================
// E23 TEST F (reload): best=250 (5/8 permanently restored) must survive a
// reload -- AND, the actual point of E23, a fresh page load must NOT pop
// every permanently-restored landmark to full current-night illumination
// just because bestNightDelivered says so. A reload is exactly like a
// fresh night in this respect: S.nightDelivered starts at 0 (S is never
// persisted, only villageProgression is), so landmark illumination must
// start dark and require tonight's own deliveries to rebuild, same as any
// other fresh start.
scenario('lumora-village-e23-best250-reload-no-autopop', null, `
__check('E23 TEST F (reload): best is preserved exactly at 250', currentVillageProgress().bestNightDelivered === 250);
__check('E23 TEST F (reload): 5/8 remain PERMANENTLY restored -- same landmark mapping, nothing lost', getRestoredMilestones('lumora').length === 5);
screen = 'village'; paused = false;
for (var rf = 0; rf < 90; rf++) __stepFrame(16); // plenty of time for an incorrect auto-pop to show itself
...
// =====================================================================
// E26 Round 2 (Jar Progression Value Rebalance): every new jar's BASE must
// already beat the previous jar's MAX, on Capacity/Range/Magnet Reach/
// Duration/Light Value. This scenario is the dedicated regression suite
// required by that phase's own spec: exact endpoints for all 6 jars (TEST
// GROUP A), monotonicity within each jar's own ladder and across jar
// transitions (the "next base > previous max" rule, no exceptions), real
// gameplay verification (not just raw numbers), and confirmation that
// nothing outside jar stats/prices moved.
scenario('lumora2-e26r2-jar-progression', { audioEnabled: true }, `
// ---- TEST GROUP A: exact base/max endpoints for all 6 jars, all 5 stats ----
var __jarTargets = {
  simple:  { capacity: [4, 7],   reach: [45, 55],   magnetReach: [110, 130], duration: [3.5, 4.25],  lightValue: [0.55, 0.70] },
  lantern: { capacity: [8, 11],  reach: [60, 70],   magnetReach: [140, 165], duration: [4.5, 5.5],   lightValue: [0.75, 0.95] },
  moon:    { capacity: [12, 15], reach: [75, 88],   magnetReach: [175, 205], duration: [5.75, 7.0],  lightValue: [1.00, 1.20] },
  crystal: { capacity: [16, 20], reach: [95, 110],  magnetReach: [215, 250], duration: [7.25, 8.5],  lightValue: [1.25, 1.50] },
  elder:   { capacity: [21, 26], reach: [118, 138], magnetReach: [260, 300], duration: [8.75, 10.0], lightValue: [1.55, 1.80] },
  aurora:  { capacity: [28, 36], reach: [145, 175], magnetReach: [315, 370], duration: [10.25, 13.0],lightValue: [1.90, 2.30] }
};
Object.keys(__jarTargets).forEach(function(key){
  var jar = JARS.find(function(j){ return j.key === key; });
  var t = __jarTargets[key];
  __check('A: ' + key + ' Capacity base/max is exactly ' + t.capacity[0] + '/' + t.capacity[1], jar.capacity === t.capacity[0] && jar.maxCapacity === t.capacity[1], 'got=' + jar.capacity + '/' + jar.maxCapacity);
  __check('A: ' + key + ' Range base/max is exactly ' + t.reach[0] + '/' + t.reach[1], jar.reachBase === t.reach[0] && jar.reachMax === t.reach[1], 'got=' + jar.reachBase + '/' + jar.reachMax);
  __check('A: ' + key + ' Magnet Reach base/max is exactly ' + t.magnetReach[0] + '/' + t.magnetReach[1], jar.magnetReachBase === t.magnetReach[0] && jar.magnetReachMax === t.magnetReach[1], 'got=' + jar.magnetReachBase + '/' + jar.magnetReachMax);
  __check('A: ' + key + ' Duration base/max is exactly ' + t.duration[0] + '/' + t.duration[1], Math.abs(jar.durationBase - t.duration[0]) < 1e-9 && Math.abs(jar.durationMax - t.duration[1]) < 1e-9, 'got=' + jar.durationBase + '/' + jar.durationMax);
  __check('A: ' + key + ' Light Value base/max is exactly ' + t.lightValue[0] + '/' + t.lightValue[1], Math.abs(jar.lightValueBase - t.lightValue[0]) < 1e-9 && Math.abs(jar.lightValueMax - t.lightValue[1]) < 1e-9, 'got=' + jar.lightValueBase + '/' + jar.lightValueMax);
});

// ---- TEST GROUP B: monotonicity -- every level N+1 >= level N, for every jar, every stat (Capacity/Range/Magnet/Light; Duration is flat-step, monotonic by construction) ----
JARS.forEach(function(jar){
  var capLevels = CAPACITY_LEVELS[jar.key];
  for (var i = 1; i < capLevels.length; i++) { __check('B: ' + jar.key + ' Capacity level ' + i + ' never decreases from level ' + (i - 1), capLevels[i] >= capLevels[i - 1], 'prev=' + capLevels[i - 1] + ' got=' + capLevels[i]); }
  ['reach', 'magnetReach', 'lightValue'].forEach(function(statKey){
    var levels = STAT_LEVELS[statKey][jar.key];
    for (var j = 1; j < levels.length; j++) { __check('B: ' + jar.key + ' ' + statKey + ' level ' + j + ' never decreases from level ' + (j - 1), levels[j] >= levels[j - 1] - 1e-9, 'prev=' + levels[j - 1] + ' got=' + levels[j]); }
  });
});

// ---- TEST GROUP B2: transition proof -- next jar's BASE strictly exceeds previous jar's MAX, for every stat, no exceptions ----
var __jarOrder = ['simple', 'lantern', 'moon', 'crystal', 'elder', 'aurora'];
for (var __ti = 0; __ti < __jarOrder.length - 1; __ti++) {
  var __prev = JARS.find(function(j){ return j.key === __jarOrder[__ti]; });
  var __next = JARS.find(function(j){ return j.key === __jarOrder[__ti + 1]; });
  var __label = __jarOrder[__ti] + ' -> ' + __jarOrder[__ti + 1];
  __check('B2: ' + __label + ' Capacity (max ' + __prev.maxCapacity + ' < base ' + __next.capacity + ')', __next.capacity > __prev.maxCapacity);
  __check('B2: ' + __label + ' Range (max ' + __prev.reachMax + ' < base ' + __next.reachBase + ')', __next.reachBase > __prev.reachMax);
  __check('B2: ' + __label + ' Magnet Reach (max ' + __prev.magnetReachMax + ' < base ' + __next.magnetReachBase + ')', __next.magnetReachBase > __prev.magnetReachMax);
  __check('B2: ' + __label + ' Duration (max ' + __prev.durationMax + ' < base ' + __next.durationBase + ')', __next.durationBase > __prev.durationMax);
  __check('B2: ' + __label + ' Light Value (max ' + __prev.lightValueMax + ' < base ' + __next.lightValueBase + ')', __next.lightValueBase > __prev.lightValueMax);
}

// ---- TEST GROUP C: real gameplay verification, not just numbers ----
// C1: Capacity is actually enforced by jarCanAcceptCatch(), using the jar's own CAPACITY_LEVELS value, not a stale/hardcoded one.
upgrades.ownedJars.simple = true; upgrades.equippedJar = 'simple'; upgrades.jarCapTiers.simple = 0;
reset(); screen = 'play'; paused = false; S.over = false;
__check('C1: a fresh Simple jar (capacity 4) accepts catches until genuinely full, then refuses', (function(){
  var accepted = 0;
  for (var c = 0; c < 6; c++) { if (jarCanAcceptCatch()) { S.carried.push({ type: 'y', ph: 0, sp: 1 }); accepted++; } }
  return accepted === 4 && S.carried.length === 4 && jarCanAcceptCatch() === false;
})(), 'carried=' + S.carried.length);
S.carried = [];

// C2: Range actually gates the real catch-lock radius (jarReachForRun()), proven via a live firefly placement, not just the raw stat.
upgrades.reachTiers.simple = 0;
reset(); screen = 'play'; paused = false;
var __c2Fly = { type: 'y', state: 'drift', x: S.jar.x, y: (S.jar.y - 14) - 50, r: 6, ph: 0 };
S.flies = [__c2Fly];
__stepFrame(16);
__check('C2: a firefly at 50px is caught by Simple\\'s new 45-55 range once actually inside it (not the old 55-64 range)', __c2Fly.state === 'drift', 'state=' + __c2Fly.state + ' (expected still drift -- 50 > base 45, proven not-yet-in-base-range)');
S.flies = [];

// C3: Duration actually times the magnet buff (already proven above with real spawnMagnet()/S.magnetT integration -- cross-referenced here against the live jarStatMax()).
__check('C3: Simple\\'s own real magnet-buff ceiling (jarStatMax) is exactly the new target (4.25s)', Math.abs(jarStatMax('duration', JARS.find(function(j){ return j.key === 'simple'; })) - 4.25) < 1e-9);

// C4: Light Value actually multiplies real delivered coins (cross-referenced against the live delivery test above, not re-derived here).
__check('C4: coinMultiplierForRun() (the separate Coin Value line) is untouched by this phase -- jar Light Value and Coin Value remain two independent multipliers, never double-applied', coinMultiplierForRun() === 1);

upgrades.jarCapTiers.simple = 0; upgrades.reachTiers.simple = 0;

// ---- TEST GROUP D: nothing outside jar stats/prices moved ----
__check('D: jar unlock prices are exactly unchanged (0/1500/3500/7500/15000/30000)', JSON.stringify(JARS.map(function(j){ return j.price; })) === JSON.stringify([0, 1500, 3500, 7500, 15000, 30000]));
__check('D: Lumora\\'s milestone thresholds are exactly unchanged (50/100/150/200/250/300/400/500)', JSON.stringify(VILLAGE1_MILESTONES.map(function(m){ return m.threshold; })) === JSON.stringify([50, 100, 150, 200, 250, 300, 400, 500]));
__check('D: Moonfall\\'s milestone thresholds are exactly unchanged', JSON.stringify(MOONFALL_MILESTONES.map(function(m){ return m.threshold; })) === JSON.stringify([100, 200, 300, 425, 550, 700, 850, 1000]));
...
// ===== E38: Workshop Tokens removed completely -- Collector remains, with no reward, unchanged otherwise =====
scenario('e38-workshop-token-removal', { audioEnabled: true }, `
__spy.loadResolve(JSON.stringify({})); // resolve the initial cloud load first -- saveProgress() below defers until this happens, same as any real Playables session
return __tick(5).then(function(){
upgrades.tutorialDone = true;
reset(); screen = 'play'; paused = false; S.isNewNight = false; S.newNightT = 999;

// ---- 1: Workshop Token state is no longer active gameplay state ----
__check('1: workshopTokens is not a defined variable anywhere in scope', typeof workshopTokens === 'undefined');

// ---- 2/3/4: Collector grants no token, cannot create/increment any balance, finalizeNight() cannot grant one ----
activeContract = 3; // collector
var coinsBefore = coins, contractsCompletedBefore = contractsCompleted.length;
var threw = false;
try { finalizeNight(); } catch (e) { threw = true; }
__check('2/4: completing a Collector night through finalizeNight() does not throw and grants no coins (no replacement reward)', !threw && coins === coinsBefore);
__check('3: contractsCompleted still records the contract (unrelated tracking), but nothing token-shaped was created -- there is no token field on window/scope to even inspect', contractsCompleted.length === contractsCompletedBefore + 1 && typeof workshopTokens === 'undefined');
activeContract = -1;

// ---- 7: cloud-save serialization does not actively serialize Workshop Tokens ----
reset(); screen='play';
saveProgress();
var lastPayload = JSON.parse(__spy.saveDataCalls[__spy.saveDataCalls.length - 1]);
__check('7: the saved cloud payload has no workshopTokens key at all', !Object.prototype.hasOwnProperty.call(lastPayload, 'workshopTokens'));

// ---- 9/10: no Workshop Token UI/reward text is rendered anywhere the Collector card draws ----
(function(){
  screen = 'contract'; contractSel = 3; contractExitAt = -1; contractScreenAt = 2;
  var calls = [];
  var orig = ctx.fillText;
  ctx.fillText = function(text){ calls.push(text); return orig.apply(ctx, arguments); };
  var drawThrew = false;
  try { draw(); } catch (e) { drawThrew = true; } finally { ctx.fillText = orig; }
  __check('9/10 setup: the contract screen (with Collector selected) renders without throwing', !drawThrew);
  var joined = calls.join(' | ');
  __check('9: no "Workshop Token" text is rendered anywhere on the contract screen', joined.indexOf('Workshop Token') === -1 && joined.indexOf('Workshop token') === -1);
  __check('10: no token reward glyph/label ("TOKEN", "✦ Workshop", the old reward icon text) is rendered', joined.indexOf('TOKEN') === -1);
  screen = 'play';
})();
(function(){
  // Night Complete's own contract result row, with Collector as tonight's contract
  S.over = true; S.overT = 1; S.tip = NIGHT_TIPS[0]; coinsAtRoundStart = coins; S.coinsEarnedThisNight = 0;
  activeContract = 3;
  var calls = [];
  var orig = ctx.fillText;
  ctx.fillText = function(text){ calls.push(text); return orig.apply(ctx, arguments); };
  var drawThrew = false;
  try { drawOver(); } catch (e) { drawThrew = true; } finally { ctx.fillText = orig; }
  __check('9/10 (b) setup: Night Complete renders without throwing with Collector as tonight\\'s contract', !drawThrew);
  var joined = calls.join(' | ');
  __check('9 (b): Night Complete shows no "Workshop Token" text for a Collector night', joined.indexOf('Workshop Token') === -1 && joined.indexOf('Workshop token') === -1);
  __check('10 (b): Night Complete shows no token reward glyph ("TOKEN") for a Collector night -- it now uses the same coin figure every other contract already used (here, +0, since Collector has no coinMult)', joined.indexOf('✦ TOKEN') === -1);
  activeContract = -1; screen = 'play';
})();

// ---- 11/12/14: Collector still exists, Playful x2 unchanged, risk unchanged ----
var collector = CONTRACTS.find(function(c){ return c.id === 'collector'; });
__check('11: Collector still exists as a real contract', !!collector && collector.name === 'COLLECTOR');
__check('12: Collector\\'s Playful spawn multiplier is still exactly 2, unchanged', collector.playfulMult === 2);
__check('14: Collector\\'s risk is still exactly 2, unchanged', collector.risk === 2);
__check('11 (b): Collector has no tokenReward field left on it at all', !('tokenReward' in collector));

// ---- 13: Collector's E37 objective-selection fix remains intact ----
activeContract = 3; nightNumber = 1; cachedNightObjectivesFor = -1; cachedNightObjectives = null;
generateNightObjectives(null);
var catchObj = S.objectiveActive.find(function(o){ return o.category === 'catch'; });
__check('13: Collector still prefers catch_playful for its forced catch objective -- the E37 fix was not regressed', !!catchObj && catchObj.id === 'catch_playful');
activeContract = -1;

// ---- 15: other contracts remain completely unchanged ----
var peaceful = CONTRACTS.find(function(c){ return c.id === 'peaceful'; });
var rush = CONTRACTS.find(function(c){ return c.id === 'rush'; });
var moth = CONTRACTS.find(function(c){ return c.id === 'moth'; });
__check('15: Peaceful is unchanged (speed -25%, +2 misses, +20% coins, risk 1)', peaceful.speedMult === 0.75 && peaceful.missBonus === 2 && peaceful.coinMult === 1.20 && peaceful.risk === 1);
__check('15 (b): Rush is unchanged (spawn +60%, rare +50%, +40% coins, risk 3)', rush.spawnMult === 1.60 && rush.rareMult === 1.50 && rush.coinMult === 1.40 && rush.risk === 3);
__check('15 (c): Moth is unchanged (moths x3, rare x2, +65% coins, risk 3)', moth.mothMult === 3 && moth.rareMult === 2 && moth.coinMult === 1.65 && moth.risk === 3);
__check('15 (d): CONTRACTS still has exactly 4 entries -- none added, none removed', CONTRACTS.length === 4);
return true;
});
`);

// ===== E38 TEST 5/6: non-YT local reload -- an old save containing workshopTokens must be safely ignored, never recreated =====
scenario('e38-workshop-token-reload', null, `
// seeded (via the localStorage seed chain below): gk2_best=40, and
// gk2_lumora2 still contains an old workshopTokens field (as a genuine
// pre-E38 save would) alongside otherwise-normal fields.
__check('5: after a genuine reload from a save whose gk2_lumora2 blob still contains an old workshopTokens field, the variable is still not defined -- the field was simply ignored, not converted/compensated', typeof workshopTokens === 'undefined');
__check('6: nothing else on the loaded save was disturbed by the presence of the stale field -- best loaded correctly', best === 40);
__check('6 (b): nightNumber (a sibling field in the SAME gk2_lumora2 blob as the stale workshopTokens key) also loaded correctly, proving the stale key did not corrupt parsing of the rest of the object', nightNumber === 7);
`);

// ===== E38 TEST 8: YT cloud-save reload -- an old cloud payload containing workshopTokens must be safely ignored, never recreated =====
scenario('e38-workshop-token-cloud-reload', { audioEnabled: true }, `
__spy.loadResolve(JSON.stringify({ best: 40, coins: 100, nightNumber: 7, workshopTokens: 12 })); // a genuine pre-E38 cloud save, workshopTokens included exactly as it would really be saved
return __tick(5).then(function(){
  __check('8: after loading an old cloud-save payload that still contains workshopTokens, the variable is still not defined -- the field was simply ignored, not converted/compensated', typeof workshopTokens === 'undefined');
  __check('8 (b): sibling fields in the SAME payload loaded correctly, proving the stale key did not corrupt loading of the rest of the object', best === 40 && coins === 100 && nightNumber === 7);
});
`);
...
async function main() {
  ...
  for (const sc of scenarios) {
    ...
    const seed = sc.name === 'standalone' ? `try{ localStorage.setItem('gk2_best','7'); }catch(e){}\n`
      ...
      // E38 TEST 5/6: a genuine pre-E38 save -- best=40, gk2_lumora2 still
      // contains an old workshopTokens field (as a real save from before this
      // removal would) alongside a normal sibling field, nightNumber:7. The
